...
- [x] HUD status chip rewritten to say only what you can play on: **"Jogando carta X"** over
  **"N pedidos"**, with the hand's size — the total both are counted against — said once, big and
...
- [x] The canvas is painted on `<html>` (`globals.css`): on iOS the rubber-band gutters, the strip behind Safari's collapsing toolbar and the slack a standalone launch leaves at the bottom all come from the *root* background — and the sky lived only on `<body>`, where `background-attachment: fixed` breaks the propagation to the canvas, so those bands rendered white. `color-scheme: dark` goes with it, otherwise Safari reads the page as light and tints its own status bar and toolbar to match. `--canvas` is the bottom bar's own base (`#0b1120`, the manifest's theme colour) because the slack always opens directly under that bar
- [x] `components/StandaloneViewport` corrects the *size* of that slack: iOS lays a standalone launch out in a viewport one status-bar inset shorter than the web view (measured on an iPhone 16 Pro: 812pt of layout inside an 874pt screen, `bottom: 0` landing at 812), and only re-measures once you scroll. When the gap is real and inset-sized it adds WebKit's legacy `height=device-height` to the viewport meta, which pins the viewport to the screen. `/dev/viewport` is the probe that found it — every viewport number live, with rails pinned to the top and bottom of the layout viewport. If the descriptor doesn't take, it falls back to performing the scroll itself: the page is exactly as tall as the short viewport, so it lends the document the missing strip as scrollable room, scrolls, and hands it back — up to three goes, stopping the moment the gap closes
