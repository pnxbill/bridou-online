...
- [x] The canvas is painted on `<html>` (`globals.css`): on iOS the rubber-band gutters, the strip behind Safari's collapsing toolbar and the safe-area slack a standalone launch shows before `100dvh` settles all come from the *root* background — and the sky lived only on `<body>`, where `background-attachment: fixed` breaks the propagation to the canvas, so those bands rendered white. `color-scheme: dark` goes with it, otherwise Safari reads the page as light and tints its own status bar and toolbar to match
