...
- [x] `metadata.appleWebApp` in `layout.tsx` (`capable`, `statusBarStyle: 'black-translucent'`, `title: 'Bridou'`). Next only emits the standardized `<meta mobile-web-app-capable>` (iOS 16.4+), so the legacy `apple-mobile-web-app-capable` alias is added via `metadata.other`. Translucent means the felt runs under the status bar, so `viewport.viewportFit: 'cover'` is on and the top/bottom `env(safe-area-inset-*)` are now real values — picked up by the header, the settings cog, the home greeting chip and the voice dock (the game table already handled its own)
