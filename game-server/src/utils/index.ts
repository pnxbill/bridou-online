...
class Utils {
  ...
  static getCardValue(card: string) {
    return isNaN(Number(card))
      ? this.cardValues[card as keyof typeof this.cardValues]
      : Number(card);
  }
  ...
  static async processLineByLine(
    ...
    for await (const line of rl) {
      ...
      if (isCard) {
        const playerIndex = GameController.games[gameId].players.findIndex(
          (p) => p.id === playerId
        );
        const playerCards =
          GameController.games[gameId].currentRound.players[
            playerIndex
          ].cards?.join(', ');
        if (playerCards === line) {
          socketInstance.to(socketId).emit('log', line);
